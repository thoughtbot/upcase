...
feature "User creates a subscription" do
  ...
  scenario "sees that the subscription is per year", js: true do
    IndividualPlan.any_instance.stubs(subscription_interval: "year")
    visit_plan_checkout_page
  ...
  scenario "changes subscription plan" do
    new_plan = create(:plan, sku: IndividualPlan::PRIME_29_SKU)
    sign_in_as_user_with_subscription
    visit my_account_path
    ...
    expect(FakeStripe.customer_plan_id).to eq IndividualPlan::PRIME_29_SKU
  end
